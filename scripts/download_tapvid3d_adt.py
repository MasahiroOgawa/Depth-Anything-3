...
def _streamed_download(url: str, dest: Path, max_attempts: int = 8) -> None:
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        if have:
            print(
                f"[adt] resuming {dest.name} from {have / 1e9:.2f} GB "
...
def main() -> int:
    ...
    ap.add_argument(
        "--batches", nargs="+", default=["0"], help="Batch indices to download (or 'all')."
    )
    ap.add_argument(
        "--keep-tarballs", action="store_true", help="Keep .tar.gz files after extraction."
    )
    args = ap.parse_args()
